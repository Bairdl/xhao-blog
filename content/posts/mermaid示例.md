...
| 类型 | 语法 | 描述 | 图形 |
| :--- | :--- | :--- | :--- |
| **同步消息（Sync）** | `->>` | 实线箭头，表示调用并等待响应（如函数调用）。 | ──▶ |
| **异步消息（Async）** | `->` | 开放式箭头，表示发送后不等待（如事件触发、消息推送）。 | ──▷ |
| **返回消息（Return）** | `-->>` 或 `-->` | 虚线箭头，表示返回值，通常可省略（Mermaid 会自动推断）。 | ──┤ 或 ──┤ |
| **自调用（Self-call）** | `->>+` | 同步自调用，激活自身栈帧。 | ↓（自身） |
| **销毁对象** | `x` 或 `X` | 表示对象被销毁，如 `A->>B: msg` `B-xB:` | ✘ |
